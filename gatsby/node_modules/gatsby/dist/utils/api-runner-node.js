...
function extendLocalReporterToCatchPluginErrors({
  ...
  if (pluginName && (reporter === null || reporter === void 0 ? void 0 : reporter.setErrorMap)) {
    setErrorMap = errorMap => reporter.setErrorMap(getErrorMapWthPluginName(pluginName, errorMap));

    error = (errorMeta, error) => reporter.error(extendErrorIdWithPluginName(pluginName, errorMeta), error);

    panic = (errorMeta, error) => reporter.panic(extendErrorIdWithPluginName(pluginName, errorMeta), error);

    panicOnBuild = (errorMeta, error) => reporter.panicOnBuild(extendErrorIdWithPluginName(pluginName, errorMeta), error);
  }
...
module.exports = async (api, args = {}, {
  ...
  Promise.mapSeries(implementingPlugins, plugin => {
    ...
    if (api === `onCreateNode` && ((_gatsbyNode = gatsbyNode) === null || _gatsbyNode === void 0 ? void 0 : _gatsbyNode.unstable_shouldOnCreateNode) && // Don't bail if this api is not exported
